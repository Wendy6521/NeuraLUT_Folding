...
def generate_truth_tables(model: nn.Module, verbose: bool = False) -> None:
    ...
    for name, module in model.named_modules():
        if type(module) == SparseLinearNeq:
            ...
            if verbose:
                if module.folding_factor == 1:
                    print(
                        f"Truth tables generated for {len(module.neuron_truth_tables)} neurons"
                    )
                else:
                    print(
                        f"Truth tables generated for {len(module.neuron_truth_tables)/2} neurons"
                    )
    ...
    for i in range(len(module_list)):
        ...
        if type(m) == SparseLinearNeq:
            ...
            module_input_bits, module_output_bits = m.gen_layer_verilog(
                module_prefix, output_directory, len(module_list), generate_bench=generate_bench
            )
            if i == 0:
                input_bitwidth = module_input_bits
            elif i == len(module_list) - 1:
                output_bitwidth = module_output_bits
            if m.folding_factor == 1:
                fold = False
            else:
                fold = True
...
class SparseLinearNeq(nn.Module):
    ...
    def gen_layer_verilog(self, module_prefix, directory, layer_num, generate_bench: bool = True):
        ...
        if self.folding_factor == 1:
            layer_contents = f"module {module_prefix} (input [{total_input_bits-1}:0] M0, output [{total_output_bits-1}:0] M1);\n\n"
        else:
            layer_contents = f"module {module_prefix} (input [{total_input_bits-1}:0] M0, input clk, input rst, output [{total_output_bits-1}:0] M1);\n\n"
            if self.folding_factor == 2:
                layer_contents += f"reg ctrl; \n\n wire [{total_output_bits-1}:0] M1_temp; \n\n assign M1 = M1_temp; \n\n reg [{total_output_bits-1}:0] M1_temp_reg;\n\n"
                layer_reg = """\
always@(posedge clk) begin
    if(!rst) begin
        ...
        ctrl<=1'b0;
        M1_temp_reg <= {total_output_bits}'b0;
    end
end\n\n"""
                layer_contents += layer_reg.format(total_output_bits = total_output_bits)
            else:
                tot_layer = layer_num + (self.folding_factor - 1) * (layer_num - 2)
                counter_bits = tot_layer.bit_length()
                counter_val = f"{counter_bits}'b{format(tot_layer, f'0{counter_bits}b')}"
                ctrl_flip = 1 + (int(module_prefix.replace("layer", "")) - 1) * self.folding_factor
                ctrl_flip_bin = f"{counter_bits}'b{format(ctrl_flip, f'0{counter_bits}b')}"
...
always@(posedge clk) begin
    ...
    if((counter == {ctrl_flip_bin})) begin
        ctrl <= 1'b0;
    end else begin
        ctrl <= 1'b1;
    end
end\n\n"""
                layer_contents += layer_reg.format(
                    total_output_bits=total_output_bits,
                    counter_bits=counter_bits,
                    counter_val=counter_val,
                    ctrl_flip_bin=ctrl_flip_bin
                )
        output_offset = 0
        for index in range(self.out_features):    
            
            wire_name_inst = f"N{index}_wire"
            ...
            if self.folding_factor == 1:
                ...
                wire_dec = f"wire [{len(indices)*input_bitwidth-1}:0] {wire_name_inst};\n"
                
            else:
                indices = []
                for i in range(2):
                    indices_single, _, _, _ = self.neuron_truth_tables[i * self.out_features + index]
                    indices.append(indices_single)
                wire_name = []
                
                for i in range (2):
                    # wire_name.append(f"{module_name}_wire")
                    wire_name.append(f"{module_prefix}_{i}_N{index}_wire")
                    # connection_line2 = f"wire [{len(indices)*input_bitwidth-1}:0] {wire_name2} = {{{connection_string2}}};\n"
                    # mux_line = f"assign {wire_name_inst} = (ctrl == 1'b0) ? {wire_name} : {wire_name2};\n"
                connection_string = generate_neuron_connection_verilog(
                    indices[0], input_bitwidth, False
                )  # Generate the string which connects the synapses to this neuron
                wire_dec = f"wire [{len(indices[0])*input_bitwidth-1}:0] {wire_name_inst};\n"
                connection_string2 = generate_neuron_connection_verilog(
                    indices[1], input_bitwidth, True
                )
                if self.folding_factor == 2:
                    mux_line = f"assign {wire_name_inst} = (ctrl == 1'b0) ? {wire_name[1]} : {wire_name[0]};\n"
                else:
                    mux_line = f"assign {wire_name_inst} = (ctrl == 1'b0) ? {wire_name[0]} : {wire_name[1]};\n"
                connection_line = f"wire [{len(indices[0])*input_bitwidth-1}:0] {wire_name[0]} = {{{connection_string}}};\n"
                connection_line2 = f"wire [{len(indices[1])*input_bitwidth-1}:0] {wire_name[1]} = {{{connection_string2}}};\n"
                inst_line = f"{module_name} {module_name}_inst (.M0({wire_name_inst}), .M1(M1_temp[{output_offset+output_bitwidth-1}:{output_offset}]));\n\n"
                layer_contents += wire_dec + connection_line + connection_line2 + mux_line + inst_line
    ...
    def lut_forward(self, x: Tensor, num_iter: int) -> Tensor:
        if self.cuda:
            x = x.cuda()
        ...
        for i in range(self.out_features):
            if num_iter == -1 or num_iter == -0:
                ...
                (
                    ...
                    float_output_states,
                    bin_output_states,
                ) = self.neuron_truth_tables[self.out_features + i]
            if self.cuda:
    ...
    def forward(self, x: Tensor, num_iter: int = -1) -> Tensor:
        if self.is_lut_inference:
            x = self.lut_forward(x, num_iter)
        else:
            if self.apply_input_quant:
