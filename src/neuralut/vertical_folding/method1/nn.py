...
def module_list_to_verilog_module(
    ...
    while (i < len(module_list)):
        ...
        if type(m) == SparseLinearNeq:
            ...
            module_prefix2 = f"layer{i+1}"
            module_input_bits, module_output_bits = m.gen_layer_verilog(
                module_prefix, module_prefix2, output_directory, m_1, generate_bench=generate_bench
            )
            if i == 0:
                input_bitwidth = module_input_bits
            elif (i == len(module_list) - 1) or (i+1 == len(module_list) - 1):
                output_bitwidth = module_output_bits
            module_contents += layer_connection_verilog(
                module_prefix,
                input_string=f"M{i}",
                input_bits=module_input_bits,
                output_string=f"M{i+2}",
                output_bits=module_output_bits,
                output_wire=(i != len(module_list) - 1) and (i+1 != len(module_list) - 1),
                register=add_registers,
            )
            if (
                i < len(module_list) - 1
                and module_list[i].out_features == module_list[i + 1].out_features
            ):
                i = i + 1
            else:
                pass
...
class SparseLinearNeq(nn.Module):
    ...
    def gen_layer_verilog(self, module_prefix, module_prefix2, directory, next_layer, generate_bench: bool = True):
        _, input_bitwidth = self.input_quant.get_scale_factor_bits()
        _, input_bitwidth2 = next_layer.input_quant.get_scale_factor_bits()
        _, output_bitwidth = self.output_quant.get_scale_factor_bits()
        _, output_bitwidth2 = next_layer.output_quant.get_scale_factor_bits()
        input_bitwidth, output_bitwidth = int(input_bitwidth), int(output_bitwidth)
        input_bitwidth2, output_bitwidth2 = int(input_bitwidth2), int(output_bitwidth2)
        total_input_bits = self.in_features * input_bitwidth
        total_input_bits2 = next_layer.in_features * input_bitwidth2
        total_output_bits = self.out_features * output_bitwidth
        total_output_bits2 = next_layer.out_features * output_bitwidth2
...
always@(posedge clk) begin
    if(!rst) begin
        ...
        for index in range(self.out_features):
            ...
            module_name2 = f"{module_prefix2}_N{index}"
            indices, _, _, _ = self.neuron_truth_tables[index]
            indices2, _, _, _ = next_layer.neuron_truth_tables[index]
            neuron_verilog = self.gen_neuron_verilog(
                index, module_name, next_layer
            ...
            connection_string2 = generate_neuron_connection_verilog(
                indices2, input_bitwidth2, True
            )
            wire_name = f"{module_name}_wire"
            wire_name2 = f"{module_name2}_wire"
            ...
            inst_line = f"{module_name} {module_name}_inst (.M0({wire_name_inst}), .ctrl(ctrl), .M1(M1_temp[{output_offset+output_bitwidth-1}:{output_offset}]));\n\n"
    ...
    # TODO: Move the verilog string templates to elsewhere
    # TODO: Move this to another class
    def gen_neuron_verilog(self, index, module_name, next_layer):
        (
            indices,
            input_perm_matrix,
            float_output_states,
        ...
        for i in range(num_entries):
            ...
            lut_string += f"\t\t\t{int(cat_input_bitwidth + 1)}'b0{entry_str}: M1r = {int(output_bitwidth)}'b{res_str};\n"
        if next_layer:
            (
                indices,
                input_perm_matrix,
                float_output_states,
                bin_output_states,
            ) = next_layer.neuron_truth_tables[index]
            _, input_bitwidth = next_layer.input_quant.get_scale_factor_bits()
            _, output_bitwidth = next_layer.output_quant.get_scale_factor_bits()
            cat_input_bitwidth = len(indices) * input_bitwidth
            ...
            for i in range(num_entries):
                ...
                for idx in range(len(indices)):
                    val = input_perm_matrix[i, idx]
                    entry_str += next_layer.input_quant.get_bin_str_from_int(val, is_cuda=self.cuda)
                res_str = next_layer.output_quant.get_bin_str_from_int(bin_output_states[i], is_cuda=self.cuda)
                lut_string += f"\t\t\t{int(cat_input_bitwidth + 1)}'b1{entry_str}: M1r = {int(output_bitwidth)}'b{res_str};\n"
        else:
            pass
