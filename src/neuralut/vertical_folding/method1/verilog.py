...
assign {input_string}w = {input_string};\n"""
    ...
    layer_connection_template += "{layer_string} {layer_string}_inst (.M0({input_string}w), .clk(clk), .rst(rst), .M1({output_string}));\n"
...
def generate_lut_verilog(module_name, input_fanin_bits, output_bits, lut_string):
    lut_neuron_template = """\
module {module_name} ( input [{input_fanin_bits_1:d}:0] M0, input ctrl, output [{output_bits_1:d}:0] M1 );

    wire [{input_fanin_bits:d}:0] M0_cat;
    assign M0_cat = {{ctrl, M0}};
...
{lut_string}
    ...
    return lut_neuron_template.format(  module_name=module_name,
                                        ...
                                        input_fanin_bits = input_fanin_bits,
                                        output_bits_1=output_bits-1,
                                        lut_string=lut_string)

def generate_neuron_connection_verilog(input_indices, input_bitwidth, next_layer):
    connection_string = ""
    for i in range(len(input_indices)):
        ...
        for b in reversed(range(input_bitwidth)):
            if next_layer:
                connection_string += f"M1_temp_reg[{offset+b}]"
            else:
