...
class FeatureMask(nn.Module):
    def __init__(self, in_features: int, out_features: int, fan_in: int, cuda: bool, folding_factor: int = 1,):
    ...
    def reset_parameters(self) -> None:
        assert self.in_features % self.folding_factor == 0, "in_features must be divisible by folding_factor"
        assert self.out_features % self.folding_factor == 0, "out_features must be divisible by folding_factor"

        in_block = self.in_features // self.folding_factor
        out_block = self.out_features // self.folding_factor

        for i in range(self.out_features):
            fold_index = i // out_block  # Determine which fold this output neuron belongs to
            input_start = fold_index * in_block

            # Sample fan_in connections from this fold's input range
            self.imask[i, :] = torch.randperm(in_block)[:self.fan_in] + input_start
