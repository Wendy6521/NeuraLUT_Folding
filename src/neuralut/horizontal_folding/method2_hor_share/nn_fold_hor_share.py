...
class SparseLinearNeq(nn.Module):
    def __init__(
        ...
        self.folded_out_features = out_features // folding_factor
        self.out_features = out_features
        self.width_n = width_n
        self.relu = nn.ReLU()

        # Define base (folded) layers
        self.fc1 = SparseLinear(fan_in, self.folded_out_features * width_n)
        self.fc2 = SparseLinear(width_n, self.folded_out_features * width_n)
        self.fc3 = SparseLinear(width_n, self.folded_out_features * width_n)
        self.fc4 = SparseLinear(width_n, self.folded_out_features)

        self.res0 = SparseLinear(fan_in, self.folded_out_features * width_n)
        self.res1 = SparseLinear(width_n, self.folded_out_features)

        # If folding is used, duplicate weights to create shared neurons
        if folding_factor > 1:
            self.fc1 = self.share_weights(self.fc1, fan_in, width_n * out_features)
            self.fc2 = self.share_weights(self.fc2, width_n, width_n * out_features)
            self.fc3 = self.share_weights(self.fc3, width_n, width_n * out_features)
            self.fc4 = self.share_weights(self.fc4, width_n, out_features)
            self.res0 = self.share_weights(self.res0, fan_in, width_n * out_features)
            self.res1 = self.share_weights(self.res1, width_n, out_features)

    def share_weights(self, base_layer, in_features, full_out_features):
        """
        Create a new SparseLinear layer with weights and biases repeated to match folding factor.
        """
        new_layer = SparseLinear(in_features, full_out_features, bias=base_layer.bias is not None)
        with torch.no_grad():
            # Repeat weights and biases folding_factor times
            new_weight = base_layer.weight.data.repeat(self.folding_factor, 1)
            new_layer.weight.data.copy_(new_weight)

            if base_layer.bias is not None:
                new_bias = base_layer.bias.data.repeat(self.folding_factor)
    ...
    def gen_layer_verilog(self, module_prefix, directory, generate_bench: bool = True):
        ...
        for index in range(num_fold_groups):
            ...
            out_wire_name = f"{module_name}_out"
            folded_output_wires[index] = out_wire_name
            ...
            if self.folding_factor == 1:
                ...
                out_wire_line = f"wire [{output_bitwidth-1}:0] {out_wire_name};\n"
                inst_line = f"{module_name} {module_name}_inst (.M0({wire_name}), .M1({out_wire_name}));\n\n"
                layer_contents += connection_line + out_wire_line + inst_line
            
            
        if self.folding_factor != 1:
            for i in range(self.out_features):
                fold_idx = i % num_fold_groups
                out_wire_name = folded_output_wires[fold_idx]
                layer_contents += f"assign M1[{output_offset+output_bitwidth-1}:{output_offset}] = {out_wire_name};\n"
                output_offset += output_bitwidth
    ...
    def lut_forward(self, x: Tensor) -> Tensor:
        ...
        for i in range(self.out_features):
            ...
            (
                ...
                float_output_states,
                bin_output_states,
            ) = self.neuron_truth_tables[fold_idx]
