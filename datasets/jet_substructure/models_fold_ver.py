...
class JetSubstructureNeqModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.folding_factor = model_config["folding_factor"]
        layer_list = []
        print("fold hidden layers")
            ...
            if i == 1:
                ...
                if self.folding_factor == 1:
                    ...
                    if in_features == out_features:
                        imask_single = FeatureMask(
                            in_features,
                            out_features,
                            fan_in=model_config["hidden_fanin"],
                            cuda=model_config["cuda"],
                        )
                        imask.append(imask_single)
                        imask.append(imask_single)
                    else:
                        imask.append(
                            FeatureMask(
                ...
                layer = SparseLinearNeq(
                    ...
                    input_quant=layer_list[-1].output_quant,
                    # output_quant=output_quant,
                    output_quant=layer_list[-1].output_quant,
                    imask=imask,
                    fan_in=model_config["hidden_fanin"],
                    width_n=model_config["width_n"],
    ...
    def verilog_forward(self, x):
        ...
        for i in range(x.shape[0]):
            ...
            if self.folding_factor <= 2:
                latency = self.latency + (self.folding_factor - 1) * (self.latency - 3) + 1
            else:
                latency = self.latency + (self.folding_factor - 1) * (self.latency - 3)
    ...
    def pytorch_forward(self, x):
        if self.folding_factor == 1:
            ...
            for l in self.module_list[1:-1]:
                for i in range(self.folding_factor):
                    # print(i)
                    x = l(x, i)
                    # print(f"Iteration {i}, Shape: {x.shape}, Min: {x.min()}, Max: {x.max()}")
