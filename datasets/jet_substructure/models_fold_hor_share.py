...
class FoldedBatchNormDuplicate(nn.Module):
    def __init__(self, out_features, folding_factor):
        super().__init__()
        assert out_features % folding_factor == 0, "out_features must be divisible by folding_factor"
        self.out_features = out_features
        self.folding_factor = folding_factor
        self.bn = nn.BatchNorm1d(out_features // folding_factor)

    def forward(self, x):
        # Take only the first block (e.g., first 1/4 if folding_factor = 4)
        base = x[:, :self.out_features // self.folding_factor]
        normed = self.bn(base)

        # Repeat the normalized part across the feature dimension
        output = normed.repeat(1, self.folding_factor)
...
class JetSubstructureNeqModel(nn.Module):
    def __init__(self, model_config):
        ...
        for i in range(1, len(self.num_neurons)):
            ...
            if i == 1:
                ...
                bn = FoldedBatchNormDuplicate(out_features, model_config["folding_factor"])
                ...
                layer = SparseLinearNeq(
                    ...
                    folding_factor=model_config["folding_factor"],
                    cuda=model_config["cuda"],
                )
    ...
    def verilog_forward(self, x):
        ...
        for i in range(x.shape[0]):
            ...
            # breakpoint()
        # print("M0", self.dut.io.M0)
        # print("M4", self.dut.io.M4)
        return y
